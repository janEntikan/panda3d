...
TypeHandle SoLoudAudioManager::_type_handle;

ReMutex SoLoudAudioManager::_lock;
int SoLoudAudioManager::_active_managers = 0;
bool SoLoudAudioManager::_soloud_active = false;
ALCdevice* SoLoudAudioManager::_device = nullptr;
ALCcontext* SoLoudAudioManager::_context = nullptr;

// This is the list of all SoLoudAudioManager objects in the world.  It must
...
SoLoudAudioManager::Managers *SoLoudAudioManager::_managers = nullptr;

SoLoudAudioManager::SourceCache *SoLoudAudioManager::_al_sources = nullptr;
 ...
 * Factory Function
 */
AudioManager *Create_SoLoudAudioManager() {
  audio_debug("Create_SoLoudAudioManager()");
  return new SoLoudAudioManager;
}
...
SoLoudAudioManager() {
  ...
  // Initialization
  audio_cat.init();
  if (_active_managers == 0 || !_soloud_active) {
    _device = nullptr;
    string dev_name = select_audio_device();

    if (!dev_name.empty()) {
      // Open a specific device by name.
      audio_cat.info() << "Using SoLoud device " << dev_name << "\n";
      _device = alcOpenDevice(dev_name.c_str());

      if (_device == nullptr) {
        audio_cat.error()
          << "Couldn't open SoLoud device \"" << dev_name << "\", falling back to default device\n";
      }
    } else {
      audio_cat.info() << "Using default SoLoud device\n";
    }
    ...
    if (_device == nullptr) {
      ...
      if (_device == nullptr && dev_name != "SoLoud Soft") {
        // Try the SoLoud Soft driver instead, which is fairly reliable.
        _device = alcOpenDevice("SoLoud Soft");

        if (_device == nullptr) {
          audio_cat.error()
            << "Couldn't open default SoLoud device\n";
        }
      }
    }
  ...
  if (!_device || !_context) {
    audio_error("SoLoudAudioManager: No open device or context");
    _is_valid = false;
  } else {
    alcGetError(_device); // clear errors
 ...
 */
string SoLoudAudioManager::
select_audio_device() {
  string selected_device = soloud_device;
  ...
  if (alcIsExtensionPresent(nullptr, "ALC_ENUMERATE_ALL_EXT") == AL_TRUE) {
    ...
    if (devices) {
      if (audio_cat.is_debug()) {
        audio_cat.debug() << "All SoLoud devices:\n";
      }
  ...
  // This extension just gives us generic driver names, like "SoLoud Soft" and
  ...
  if (alcIsExtensionPresent(nullptr, "ALC_ENUMERATION_EXT") == AL_TRUE) {
    ...
    if (devices) {
      if (audio_cat.is_debug()) {
        audio_cat.debug() << "SoLoud drivers:\n";
      }
      ...
      while (*devices) {
        ...
        if (selected_device.empty() && device == "SoLoud Soft" &&
            default_device == "Generic Software") {
          // Prefer SoLoud Soft over the buggy Generic Software driver.
          selected_device = "SoLoud Soft";
        }
...
/**
 * This makes this manager's SoLoud context the current context.  Needed
 ...
 * count.
 */
SoLoudAudioManager::SoundData *SoLoudAudioManager::
get_sound_data(MovieAudio *movie, int mode) {
  ...
  if (should_load_audio(stream, mode)) {
    ...
    if (sd->_sample == 0) {
      audio_error("Could not create an SoLoud buffer object");
      delete sd;
      return nullptr;
    }
    ...
    int err = alGetError();
    if (err != AL_NO_ERROR) {
      audio_error("could not fill SoLoud buffer object with data");
      delete sd;
      return nullptr;
    }
 ...
 */
PT(AudioSound) SoLoudAudioManager::
get_sound(MovieAudio *sound, bool positional, int mode) {
  ReMutexHolder holder(_lock);
  if(!is_valid()) {
    return get_null_sound();
  }
  PT(SoLoudAudioSound) oas =
    new SoLoudAudioSound(this, sound, positional, mode);
  ...
  PT(AudioSound) res = (AudioSound*)(SoLoudAudioSound*)oas;
  return res;
}
...
get_sound(const Filename &file_name, bool positional, int mode) {
  ...
  PT(SoLoudAudioSound) oas =
    new SoLoudAudioSound(this, mva, positional, mode);
 ...
 *
 */
unsigned int SoLoudAudioManager::
 ...
 */
void SoLoudAudioManager::
release_sound(SoLoudAudioSound* audioSound) {
  ReMutexHolder holder(_lock);
 ...
 * Sets listener gain
 */
void SoLoudAudioManager::set_volume(PN_stdfloat volume) {
  ReMutexHolder holder(_lock);
  if (_volume!=volume) {
    _volume = volume;
 ...
 * Gets listener gain
 */
PN_stdfloat SoLoudAudioManager::
get_volume() const {
  return _volume;
}
 ...
 * IMPORTANT! Both Panda3D and SoLoud use a right handed coordinate system.
 * But there is a major difference!  In Panda3D the Y-Axis is going into the
 * Screen and the Z-Axis is going up.  In SoLoud the Y-Axis is going up and
 ...
 * to SoLoud and back.  What does did mean to average Panda user?  Nothing,
 ...
 * WARNING: SoLoud has no distance factor but we use this as a scale
 ...
 */
void SoLoudAudioManager::
starting_sound(SoLoudAudioSound* audio) {
  ReMutexHolder holder(_lock);
  ALuint source=0;
 ...
 */
void SoLoudAudioManager::
stopping_sound(SoLoudAudioSound* audio) {
  ReMutexHolder holder(_lock);
  if (audio->_source) {
...
cleanup() {
  ...
  if (_active_managers == 0) {
    if (_soloud_active) {
      ...
      if (_device) {
        audio_debug("Going to try to close SoLoud");
        alcCloseDevice(_device);
        // alc_audio_errcheck("alcCloseDevice(_device)",_device);
        _device = nullptr;
        audio_debug("SoLoud Closed");
      }

      _soloud_active = false;
    }
  }
...
SoLoudAudioManager::SoundData::
~SoundData() {
  ReMutexHolder holder(SoLoudAudioManager::_lock);
  if (_sample != 0) {
...
/**
 * Deletes an SoLoud buffer.  This is a special function because some
 * implementations of SoLoud (e.g. Apple's) don't unlock the buffers
...
delete_buffer(ALuint buffer) {
  ...
  while (true) {
    ...
    if (error == AL_NO_ERROR) {
      ...
      break;
    } else if (tries >= soloud_buffer_delete_retries.get_value()) {
      // We ran out of retries.  Give up.
      break;
    } else {
      // Make another try after (delay * 2^n) seconds.
      Thread::sleep(soloud_buffer_delete_delay.get_value() * (1 << tries));
      tries++;
    }
  }
