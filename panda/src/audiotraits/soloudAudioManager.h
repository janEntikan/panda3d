 ...
 *
 * @file soloudAudioManager.h
 * @author jan Entikan
 */

#ifndef SOLOUDAUDIOMANAGER_H
#define SOLOUDAUDIOMANAGER_H
...
// OSX uses the SoLoud framework
#ifdef HAVE_SOLOUD_FRAMEWORK
  #include <SoLoud/al.h>
  #include <SoLoud/alc.h>
#else
  #include <AL/al.h>
  #include <AL/alc.h>
#endif

class SoLoudAudioSound;

extern void al_audio_errcheck(const char *context);
extern void alc_audio_errcheck(const char *context,ALCdevice* device);

class EXPCL_SOLOUD_AUDIO SoLoudAudioManager : public AudioManager {
  class SoundData;

  friend class SoLoudAudioSound;
  friend class SoLoudSoundData;
 ...
 public:
  ...
  // feet, set 3.28. This factor is applied only to Fmod and SoLoud at the
  // moment.
  // SoLoud in fact has no distance factor like Fmod, but works with the speed
...
private:
  ...
  void release_sound(SoLoudAudioSound* audioSound);
  ...
  void starting_sound(SoLoudAudioSound* audio);
  void stopping_sound(SoLoudAudioSound* audio);

  void cleanup();

private:
  // This global lock protects all access to SoLoud library interfaces.
 ...
 */
  ...
  typedef phash_set<PT(SoLoudAudioSound)> SoundsPlaying;
  SoundsPlaying _sounds_playing;

  typedef phash_set<SoLoudAudioSound *> AllSounds;
  ...
  static bool _soloud_active;
  unsigned int _concurrent_sound_limit;

  bool _is_valid;

  typedef pset<SoLoudAudioManager *> Managers;
  static Managers *_managers;

  static ALCdevice* _device;
  static ALCcontext* _context;

  // cache of soloud sources, use only for playing sounds
 ...
 public:
  ...
  static void init_type() {
    ...
    register_type(_type_handle, "SoLoudAudioManager", AudioManager::get_class_type());
  }
...
EXPCL_SOLOUD_AUDIO AudioManager *Create_SoLoudAudioManager();

#endif /* SOLOUDAUDIOMANAGER_H */
