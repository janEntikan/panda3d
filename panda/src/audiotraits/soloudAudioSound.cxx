...
TypeHandle SoLoudAudioSound::_type_handle;


#ifndef NDEBUG //[
  #define soloud_audio_debug(x) \
      audio_debug("SoLoudAudioSound \""<<get_name() \
      <<"\" "<< x )
#else //][
#define soloud_audio_debug(x) ((void)0)
#endif //]
...
SoLoudAudioSound::
SoLoudAudioSound(SoLoudAudioManager* manager,
                 MovieAudio *movie,
                 bool positional,
                 int mode) :
  _movie(movie),
 ...
 */
unsigned long SoLoudAudioSound::
get_loop_count() const {
  return _loop_count;
}

/**
 * When streaming audio, the computer is supposed to keep SoLoud's queue full.
 * However, there are times when the computer is running slow and the queue
 * empties prematurely.  In that case, SoLoud will stop.  When the computer
 * finally gets around to refilling the queue, it is necessary to tell SoLoud
 * to resume playing.
 */
void SoLoudAudioSound::
restart_stalled_audio() {
...
/**
 * Creates an SoLoud buffer object.
 */
ALuint SoLoudAudioSound::
make_buffer(int samples, int channels, int rate, unsigned char *data) {
  ...
  if (alGetError() != AL_NO_ERROR) {
    audio_error("could not allocate an SoLoud buffer object");
    cleanup();
    return 0;
  }
...
/**
 * Pulls any used buffers out of SoLoud's queue.
 */
void SoLoudAudioSound::
pull_used_buffers() {
  ReMutexHolder holder(SoLoudAudioManager::_lock);
  ...
  while (_stream_queued.size()) {
    ...
    if (err == AL_NO_ERROR) {
      if (_stream_queued[0]._buffer != buffer) {
        // This is certainly atypical: most implementations of SoLoud unqueue
...
/**
 * Pushes fresh buffers into SoLoud's queue until the queue is "full" (ie, has
 * plenty of data).
 */
void SoLoudAudioSound::
push_fresh_buffers() {
...
set_balance(PN_stdfloat balance_right) {
  audio_debug("SoLoudAudioSound::set_balance() not implemented");
}
...
PN_stdfloat SoLoudAudioSound::
get_balance() const {
  audio_debug("SoLoudAudioSound::get_balance() not implemented");
  return 0;
}
 ...
 *
 * Both Panda3D and SoLoud use a right handed coordinate system.  However, in
 * Panda3D the Y-Axis is going into the Screen and the Z-Axis is going up.  In
 * SoLoud the Y-Axis is going up and the Z-Axis is coming out of the screen.
 *
 * The solution is simple, we just flip the Y and Z axis and negate the Z, as
 * we move coordinates from Panda to SoLoud and back.
 */
void SoLoudAudioSound::
 ...
 *
 */
const std::string& SoLoudAudioSound::
 ...
 */
AudioSound::SoundStatus SoLoudAudioSound::
status() const {
  ReMutexHolder holder(SoLoudAudioManager::_lock);
  if (!is_playing()) {
